...
public sealed class DashboardService : IDashboardService
{
    ...
    public DashboardService(AppDbContext db, IForecastService forecastService)
    {
        _db = db;
        ...
        CancellationToken ct = default)
    {
        ...
        {
            var comparisonYearSeries = new Dictionary<int, DashboardPerformanceSeriesLineDto>();
            foreach (var series in comparisonSeries)
            {
                var suffix = series.Id.StartsWith("year-", StringComparison.Ordinal) ? series.Id["year-".Length..] : null;
                if (suffix is not null &&
                    int.TryParse(suffix, NumberStyles.Integer, CultureInfo.InvariantCulture, out var yearOffset) &&
                    yearOffset > 0)
                {
                    comparisonYearSeries[yearOffset] = series;
                }
            }
